class puphpet_server (
  ...
  exec { 'dotfiles':
    cwd     => $user_home,
    command => "cp -r /vagrant/vendor/integrated/puphpet/files/dot/.[a-zA-Z0-9]* ${user_home}/ && \
                chown -R ${::ssh_username} ${user_home}/.[a-zA-Z0-9]* && \
                cp -r /vagrant/vendor/integrated/puphpet/files/dot/.[a-zA-Z0-9]* /root/",
    onlyif  => 'test -d /vagrant/vendor/integrated/puphpet/files/dot',
    returns => [0, 1],
